...
class ShopItem extends Component {
    ...
    return this.state.product.tags.map(tag => {
      return (
        <span key={tag.id}>
          <Link
            to={'/shop/tags/' + tag.id}
            class="text-xs mb-sm-1 mr-3 text-uppercase text-black font-weight-light"
          >
            {tag.name}
          </Link>
        </span>
        ...
        <div key={index} class="col-4 col-lg-3 d-flex p-0">
          <div class="card mb-3 border-0 flex-ew">
            ...
            <div class="card-body pb-0">
              ...
              <p class="card-text text-uppercase text-inverse text-xs">
                {r.category.name}
              </p>
            </div>
          </div>
    ...
    var ingredients = this.state.product.ingredients.map((i, index) => {
      return (
        <span key={index}>
          <a
            class="text-black"
            href={'https://www.lifelixir.com.au/research#' + i.info_link}
            target="_blank"
          >
            {i.name}
          </a>
          {i.extract ? <span class="">&#176;</span> : null}
          {this.state.product.ingredients.length - 1 !== index ? (
            <span>, </span>
          ) : null}
        </span>
      );
  ...
  imageGalleryImages() {
    ...
    images.unshift({
      original: this.state.product.photo.original,
      thumbnail: this.state.product.photo.medium,
    });
    ...
    return (
      <div id="content" class="pt-3 pt-lg-6 pb-lg-0">
        <Helmet>
          ...
          <meta
            name="description"
            content={this.state.product.description.replace(
              /(<([^>]+)>)/gi,
              ''
            )}
          />
        ...
        <div class="container">
                ...
                <div class="card-body p-0 px-4 pos-relative">
                  ...
                  <p
                    dangerouslySetInnerHTML={{
                      __html: this.state.product.short_description,
                    }}
                    className="lead"
                  />
                  <hr class="my-3" />
                  <div class="row">
                    <div class="col-auto mr-auto">
                      <h4 class="font-weight-light my-2 text-primary">
                        {this.state.product.before_discount_price !== 0 ? (
                          <span>
                            <del class="text-danger">
                              ${this.state.product.before_discount_price}
                            </del>{' '}
                            /{' '}
                          </span>
                        ) : null}
                          ...
                          <span class="text-black text-xs">
                            ...
                            {Math.round(
                              this.state.product.weight /
                                this.state.product.serving_size
                            )}{' '}
                            servings
                          </span>
                        )}
                  ...
                  <p
                    ...
                    class=""
                  />
                  <p class="mb-1">{this.renderTags()}</p>
                  <div
                    class="card-accordion card-accordion-list-style card-accordion-icons-left"
                    role="tablist"
                  ...
                  >
                    ...
                    <div class="card">
                      ...
                      <div class="collapse show">
                        <p
                          dangerouslySetInnerHTML={{
                            __html: this.state.product.serving_idea,
                          }}
                          class="text-sm"
                        />
                      </div>
                    ...
                    <div class="card">
                      ...
                      <div class="collapse show">
                        <div class="">
                          <div class="row">{this.renderRecipes()}</div>
                        </div>
                      </div>
                    </div>
                  </div>

                  <div class="my-2 text-xs font-weight-bold text-uppercase">
                    {this.renderProps()}
                  </div>
                  <div class="text-xs my-2 text-muted">
                    Note: These statements have not been evaluated by food and
          ...
          <div class="col-lg-12">
            ...
            {this.state.product.id ? (
              <ProductsCarousel
                shop={_.filter(this.props.featured, i => {
