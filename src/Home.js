...
class Home extends Component {
  ...
  renderRecipes(product) {
    return _.sortBy(product.recipes, 'order').map((r, index) => {
      return (
        <ul key={r.id}>
  ...
  renderArticles() {
    return this.props.articles.map((article, map) => {
      return (
        <div class="col-12 col-lg-4 mb-lg-1">
          <Link to={'/blog/' + article.slug} class="card border-0 flex-ew">
            ...
            <div class="card-img-overlay bg-dark bg-op-5 text-white flex-valign-b">
              <div>
                <h5 class="card-title text-white mb-1">{article.title}</h5>
                <p class="card-text text-uppercase text-white text-xs">
                  <span class="text-primary">{article.author}</span>{' '}
                  <span class="px-1">/</span>{' '}
                  {moment(article.updated_at).format('MMM Do YY')}
                </p>
              </div>
            </div>
          </Link>
  ...
  handleDownloadPdf(e) {
    ...
    render(
      <Modal title="First Step">
        <p class="text-sm">
          <p class="lead">
            Receive the 10 easiest and most effective biohacks to enhance your
            life today in your inbox.
          </p>
          <p>
            Biohacking is maximising our biological potentials. In order to hack
            our biology we have to hack our mitochondria, our energy powerhouse,
                ...
                <div class="row justify-content-center align-items-center">
                  <div class="col-lg-6 padding-bottom-1x text-md-left text-center">
                    <div class="from-bottom">
                      ...
                      <p>
                        <Link
                          class="text-sm text-uppercase btn btn-primary mr-2"
                          to="/shop"
                        >
                          Shop Now
                        </Link>
                        <Link class="text-sm text-uppercase" to="/research">
                          Learn More
                        </Link>
                      </p>
                    </div>
          ...
          <div class="container">
            <div class="row mt-4">
              <div class="col-lg-4 col-xl-5 flex-valign pos-relative">
                ...
                <p class="" />
                <p class="lead">
                  Download daily rituals and practices for optimum brain
                  function.
                  <div class="mt-2">
                    <Subscribe campaign="Download10Hacks" />
                  </div>
                ...
                <p class="text-xs text-center">
                  Breathing box is a technique to manage your stress in any
                  stressful situations.{' '}
                  <span class="font-weight-bold text-uppercase">
                    Inhale, hold, exhale, hold and repeat
                  </span>{' '}
                  until you are feeling back in control.
                </p>
              </div>
